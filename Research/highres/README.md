...
**Wellington removal: done.** Collections **1–52 terminal** (Howick 17 RE-DONE; TAPUHI 25 committed via a
...
403s on the presigned S3 leg); 0 honest-smaller, median 2.44×, max 67.76×. User-approved. **Manawatū
Heritage 52 committed Group B ADD** — the same Recollect signed-IIIF platform as Feilding (35) and Kete
Horowhenua (51), but genuinely **mixed-format** (census: 14 TIFF, 3 JPEG, 3 no-download-link of 20
sampled) — new `manawatuHeritageOriginal` strategy probes each resolved download link's Content-Type via
a ranged GET and branches direct-serve (JPEG) vs the self-hosted Pillow converter (TIFF); **required a
live AWS redeploy** (new `ALLOWED_HOSTS` entry, user-confirmed changeset, `sam build && sam deploy`); 0
honest-smaller, median ≈29×. User-approved both a direct-JPEG and a converter-routed live URL.

**★ THE SWEEP IS COMPLETE — all 52 collections are now terminal** (38 committed, 12 no-improvement, 2
blocked/removed). There is no order 53; `progress.json`'s `collections` array ends at order 52. Any
future work here is either a **re-check** of an existing entry (e.g. a `FOLLOW-UP` noted in a past log —
several collections have an open one, such as Antarctica NZ 2's email-for-access request or Presbyterian
Research Centre 5's migration) or the user explicitly adding a **new** collection to DigitalNZ's harvest
pool for the sweep to consider. Do not invent a "next order" — check with the user.
...
| 52 | Manawatū Heritage | recollectIIIF (was `boutique`) | **committed Group B ADD** — the SAME **Recollect signed-IIIF `curtis-production2-cache`** platform as **Feilding (35)** and **Kete Horowhenua (51)** (`manawatuheritage.pncc.govt.nz`, Palmerston North City Library); was **NOT in the Lambda**. **★ A THIRD outcome — neither always-TIFF nor always-JPEG, but genuinely MIXED-FORMAT**: a 20-record census of resolved `download?variant=original` endpoints found 14 TIFF, 3 JPEG, 3 no-download-link — so the format must be checked **per record at request time**, not assumed from the collection. New `manawatuHeritageOriginal` strategy resolves the download link, then a **ranged-GET Content-Type probe** (new `NetworkRequestManager.rangeContentType`, never HEAD — the S3 leg 403s that) decides direct-serve (JPEG, Kete-style) vs converter-routing (TIFF, Feilding-style). **★ This DID require a live AWS redeploy** — `manawatuheritage.pncc.govt.nz` added to the converter's `ALLOWED_HOSTS`; `sam build && sam deploy` (in-place update, 3 `Modify`/0 replace) after the user confirmed the exact CloudFormation changeset (no tty for the interactive y/N prompt in this environment, so a reviewed `--no-confirm-changeset` apply was used instead). The **~15-20% "no download link"** records are multi-page/compound "album" parents (`?child=<uuid>` sibling pages hold the actual files; the parent's own Download modal is a genuinely empty `<div>`) — falls back to the harvested baseline rather than chasing an arbitrary child page. Pixel survey (7/10 resolved): **0 honest-smaller**, **4.07×–32.28×**, median **≈29×**. `swift build` 0; `sam build`/`sam deploy` succeeded; CollectionTester ×12 local + 6 live deployed-endpoint picks confirmed **all three code paths** (fallback / direct-JPEG / converter-TIFF), including a live 2060×1760 direct-JPEG pick and a live 2624×3652 converter-routed pick. `collectionWeights` **0.002** (provisional). **User-approved 2026-07-04** (both the direct-JPEG and converter URLs). See `logs/052-manawatu-heritage.md` |
