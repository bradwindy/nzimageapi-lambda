# High-Res Sweep Worklist

Human-readable mirror of `progress.json` (the source of truth).
Regenerate with `python3 Research/highres/gen_worklist.py`.

## Removal
- [x] REMOVE Wellington City Recollect — done

## Collections (52/52 terminal)

### recollect
- [x] (01) Tauranga City Libraries Other Collection — group A — platform recollect — committed
- [x] (02) Antarctica NZ Digital Asset Manager — group A — platform recollect — no-improvement
- [x] (04) National Army Museum — group A — platform recollect — no-improvement
- [x] (05) Presbyterian Research Centre — group A — platform recollect — blocked
- [x] (06) Hastings Recollect — group B — platform recollect — committed
- [x] (07) Lower Hutt MyRecollect — group B — platform recollect — committed
- [x] (08) Hocken Digital Collections — group B — platform recollect — committed
- [x] (09) Tāmiro — group A — platform recollect — no-improvement
- [x] (10) He Purapura Marara Scattered Seeds — group A — platform recollect — committed
- [x] (36) Clutha Heritage — group B — platform recollect — committed
- [x] (37) John Kinder Theological Library — group B — platform recollect — committed
- [x] (38) Tasman Heritage — group B — platform recollect — committed
- [x] (39) Western Bay Community Archives — group B — platform recollect — committed
- [x] (41) Far North District Libraries Rediscovery — group B — platform recollect — committed
- [x] (42) Pakiaka Rotorua Heritage Online — group B — platform recollect — committed

### flickr
- [x] (11) Ministry for Culture and Heritage Te Ara Flickr — group B — platform flickr — committed
- [x] (12) Alexander Turnbull Library Flickr — group A — platform flickr — committed
- [x] (13) Dunedin City Council Archives Flickr — group B — platform flickr — committed
- [x] (14) State Library of New South Wales Flickr — group B — platform flickr — committed
- [x] (15) Australian National Maritime Museum Flickr — group B — platform flickr — committed

### iiif
- [x] (16) Kura Heritage Collections Online — group A — platform iiif — committed
- [x] (43) Victoria and Albert Museum — group B — platform iiif — committed

### ehiveIIIF
- [x] (17) Howick Historical Village NZMuseums — group A — platform ehiveIIIF — committed
- [x] (18) Mataura Museum NZMuseums — group B — platform ehiveIIIF — committed
- [x] (19) New Zealand Portrait Gallery NZMuseums — group B — platform ehiveIIIF — committed
- [x] (31) Te Hikoi Museum — group A — platform ehiveIIIF — committed
- [x] (32) Te Toi Uku, Crown Lynn and Clayworks Museum — group A — platform ehiveIIIF — committed
- [x] (33) Te Ūaka The Lyttelton Museum — group B — platform ehiveIIIF — committed
- [x] (34) Wyndham & Districts Historical Museum — group B — platform ehiveIIIF — committed

### pastPerfect
- [x] (20) Waimate Museum and Archives PastPerfect — group A — platform pastPerfect — no-improvement
- [x] (29) South Canterbury Museum — group A — platform pastPerfect — no-improvement

### weservProxy
- [x] (21) Te Papa Collections Online — group A — platform weservProxy — committed

### aklMuseumCloudimg
- [x] (22) Auckland Museum Collections — group A — platform aklMuseumCloudimg — committed

### thumbnailerProxy
- [x] (23) Auckland Libraries Heritage Images Collection — group A — platform thumbnailerProxy — blocked

### tapuhi
- [x] (25) TAPUHI — group A — platform tapuhi — committed

### stringSwap
- [x] (26) Canterbury Museum — group A — platform stringSwap — no-improvement
- [x] (27) Auckland Art Gallery Toi o Tāmaki — group A — platform stringSwap — no-improvement
- [x] (28) Culture Waitaki — group A — platform stringSwap — no-improvement

### boutique
- [x] (30) V.C. Browne & Son NZ Aerial Photograph Collection — group A — platform boutique — no-improvement

### ndha
- [x] (03) National Publicity Studios black and white file prints — group A — platform ndha — no-improvement
- [x] (40) War Art Online — group B — platform ndha — committed

### knowledgeBankMaster
- [x] (24) Hawke's Bay Knowledge Bank — group A — platform knowledgeBankMaster — committed

### recollectIIIF
- [x] (35) Feilding Library — group B — platform recollectIIIF — committed
- [x] (51) Kete Horowhenua — group B — platform recollectIIIF — committed
- [x] (52) Manawatū Heritage — group B — platform recollectIIIF — committed

### ehive
- [x] (44) The University of Waikato Art Collection — group B — platform ehive — committed

### vernonBrowser
- [x] (45) Te Ahu Museum — group B — platform vernonBrowser — committed
- [x] (46) Ngā Puhipuhi o Te Herenga Waka—Victoria University of Wellington Art Collection — group B — platform vernonBrowser — committed
- [x] (47) Nelson Provincial Museum — group B — platform vernonBrowser — committed
- [x] (48) Puke Ariki — group B — platform vernonBrowser — committed

### spydus
- [x] (49) Picture Wairarapa — group B — platform spydus — no-improvement

### teara
- [x] (50) Te Ara - The Encyclopedia of New Zealand — group B — platform teara — no-improvement
