...
if [ -n "$FUNCTION_NAME" ]; then
    ...
    echo "  - API_CLIENT_SECRETS=name:your_secret (see docs/ACCESS-CONTROL.md)"
    echo "  - LOG_LEVEL=info"
    echo ""
    echo "Or set them via CLI:"
    echo "  aws lambda update-function-configuration \\"
    echo "      --function-name $FUNCTION_NAME \\"
    echo "      --environment \"Variables={DIGITALNZ_API_KEY=your_key,API_CLIENT_SECRETS=name:your_secret,LOG_LEVEL=info}\""
else
